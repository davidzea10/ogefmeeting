...
import {
  enregistrerViewerDocument,
  retirerViewerDocument,
} from './document-broadcast.js';
...
/**
 * WebSocket lecture : reçoit l’état du document présenté (page / scroll).
 * Chemin : /ws/document-view?reunionId=...&token=...
 */
export function attachDocumentViewWebSocket(server: HttpServer): void {
  ...
  logger.info({ path: '/ws/document-view' }, 'WebSocket document viewer prêt');
}
...
async function handleViewer(client: WebSocket, req: IncomingMessage): Promise<void> {
  ...
  enregistrerViewerDocument(reunionId, client);
  ...
  client.on('close', () => {
    arreterKeepAlive();
    retirerViewerDocument(reunionId, client);
  });
